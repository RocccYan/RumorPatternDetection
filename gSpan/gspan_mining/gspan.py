...
from .utils import save_as_json, mkdir
...
class gSpan(object):
    ...
    def __init__(self,
        ...
        self.out_file_name = "gspan"+database_file_name.replace('/','.')
        self.graphs = {}
    ...
    def _report(self, projected):
        ...
        # Save report_df and _where locally.[roccc]
        save_as_json(self._report_df.to_dict(), f"./{self.out_file_name}/gSpan_dataframe.json")

        if self._visualize:
            # g.plot()
            g.plot(self.out_file_name)
        if self._where:
            gids_projected = list({p.gid for p in projected})
            # print(f'where: {gids_projected}')
            print("where gids from has been saved.")
            save_as_json(gids_projected, f'./{self.out_file_name}/{display_str}_gids.json')
        print('\n-----------------\n')
